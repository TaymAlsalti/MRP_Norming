...
# The functions has the following arguments and defaults
# age_norm_comparisons <- function(..., # brms fits
#                                  ps_table = census, # poststratification table
#                                  ps_variables = c("age", "educ", "mig", "male"), # adjustment set
#                                  re_formula = NULL, # whether some group level predictors are to be omitted
#                                  sim_size = 100000, # how large should the simulated sample be
#                                  RP = NULL,  # whether model predictions without poststratification should be shown
#                                  labels = NULL, # labels for the geom_text in the final plots
# #                                    palette = c(
#   "#BC3C29FF",
#   "#0072B5FF",
#   # "#20854EFF",
...
#   # "#EE4C97FF"
# ),  # group colours
#                                  output_file = "../02_data/results.rds") # save produced df and plots
```
...
main_educ_vs_not <- age_norm_comparisons(
  ...
  output_file = "../01_data/results/main_educ_vs_not.rds"
  )
...
rpp_nor_ints <- age_norm_comparisons(
  ...
  output_file = "../01_data/results/rpp_nor_ints.rds"
  )
...
educXmale_vs_not <- age_norm_comparisons(
  ...
  output_file = "../01_data/results/educXmale_vs_not.rds"
  )
...
```{r}
ggsave("../04_figures/S02_pp_check.png", width = 8, height = 4)
```
...
main_vs_normal <- age_norm_comparisons(
  ...
  output_file = "../01_data/results/main_vs_normal.rds"
  )
...
```{r}
ggsave("../04_figures/S04_main_vs_normal.png", main_vs_normal$means_plot, width = 8, height = 4)
```
...
main_vs_boosted <- age_norm_comparisons(
  ...
  output_file = "../01_data/results/main_vs_boosted.rds"
  )
...
```{r}
ggsave("../04_figures/S05_main_vs_boosted.png", main_vs_boosted$means_plot, width = 8, height = 4)
```
...
main_vs_ordinal <- age_norm_comparisons(
  ...
  output_file = "../01_data/results/main_vs_ordinal_ord.rds"
  )
...
```{r}
ggsave("../04_figures/S06_main_vs_ordinal.png", main_vs_ordinal$means_plot, width = 8, height = 4)
```
...
main_vs_negbin <- age_norm_comparisons(
  ...
  output_file = "../01_data/results/main_vs_negbin.rds"
  )
...
main_vs_beta <- age_norm_comparisons(
  ...
  output_file = "../01_data/results/main_vs_beta.rds"
  )
...
sim_pop_sample_with_draws_beta <- sim_pop_sample %>%
  add_predicted_draws(brm_beta_ints_no_educ_male,
                      ndraws = 100,
                      seed = 810,
                      allow_new_levels = TRUE) %>%
  mutate(.prediction = round(case_when(.prediction*57 < 0 ~ 0,
                                 .prediction*57 > 56 ~ 56,
                                 TRUE ~ .prediction*57))) 

means_sds_and_ses_RPP_beta <- sim_pop_sample_with_draws_beta %>%
  group_by(age, .draw) %>% 
  summarise(mean_prediction = mean(.prediction), 
            sd_prediction = sd(.prediction)) %>%
  group_by(age) %>% 
  summarise(RPPbeta_mean = mean(mean_prediction), 
            RPPbeta_se_of_mean = sd(mean_prediction), 
            RPPbeta_sd = sqrt(mean(sd_prediction^2)), 
            RPPbeta_se_of_sd = sd(sd_prediction)) 

sim_pop_sample_with_draws_skew <- data.table::fread("../01_data/results/sim_pop_sample_with_draws.csv.gz")

means_sds_and_ses_RPP_skew <- sim_pop_sample_with_draws_skew %>%
  group_by(age, .draw) %>% 
  summarise(mean_prediction = mean(.prediction), 
            sd_prediction = sd(.prediction)) %>%
  group_by(age) %>% 
  summarise(RPPskew_mean = mean(mean_prediction), 
            RPPskew_se_of_mean = sd(mean_prediction), 
            RPPskew_sd = sqrt(mean(sd_prediction^2)), 
            RPPskew_se_of_sd = sd(sd_prediction)) 
...
means_ns_sds_and_ses <- means_sds_and_ses_RPP_beta %>% 
  left_join(means_sds_and_ses_tl_ps, by = "age")  %>%
  left_join(means_sds_and_ses_RPP_skew, by = "age")  %>%
...
ggsave("../04_figures/S03_ppc_grouped.png", width = 8, height = 8)
```
...
ggsave("../04_figures/3_splines_ST.png", width = 9, height = 5)
```
...
ggsave("../04_figures/03_splines_ISCED.png", width = 9, height = 5, scale = 1)
...
data.table::fwrite(sim_pop_sample_with_draws, "../01_data/results/sim_pop_sample_with_draws.csv.gz")
```
