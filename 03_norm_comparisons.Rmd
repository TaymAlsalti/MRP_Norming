...
brm_beta_ints_no_educ_male <- readRDS("../unshareable_data/brms/cft/brm_beta_ints_no_educ_male.rds", )
...
RPP_vs_raw <- age_norm_comparisons(
  ...
  function(x) round(x*56) # for handling proportion (beta) predictions
),
   palette = c(
  "#BC3C29FF",
  "#0072B5FF"
...
RPP_vs_raw$percentile_plot$layers <- RPP_vs_raw$percentile_plot$layers[!vapply(RPP_vs_raw$percentile_plot$layers, function(l) {
  any(class(l$geom) %in% c("GeomText", "GeomLabel"))
}, logical(1))]

RPP_vs_raw$percentile_plot  + 
  geom_text(
    data = combined_percentiles %>% filter(age == 65, show_text, Source != "Raw"),
    aes(label = percentile_label),
    nudge_x = 1,
    hjust = 0,
    family = "Times"
  )
...
Note that calculating linearly transformed IQs doesn't make a lot of sense here since such a calculation assumes a normal distribution.  
...
iq_table %>% 
  select(age_group, raw_score, "RPP Normalised IQ" = RPP_IQ_normalised) %>% 
...
#### Percentile plot
```{r}
RPP_percentiles <- RPP_vs_raw$percentile_plot$data %>% filter(Source != "Raw") %>% mutate(line_label = percentile_label, show_text = TRUE) 
...
combined_percentiles <- RPP_percentiles %>% filter(Source != "Raw") %>% mutate(line_label = percentile_label, show_text = TRUE) %>% bind_rows(GAMLSS_MRP_percentiles) %>% bind_rows(GAMLSS_MRP_percentiles2)  
