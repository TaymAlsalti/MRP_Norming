...
ggplot(data = fitted_ST, aes(x = age, y = Estimate, color = school_type)) +
  ...
  geom_text_repel(
    data = fitted_ST %>% filter(age == max(age, na.rm = T)) %>% arrange(school_type),
    aes(label = school_type), 
    family = "Times", seed = 810, nudge_x = .3, hjust = 0, point.padding = 1, direction = "y") +
  geom_text_repel(
    data = fitted_ST %>% filter(school_type == "Primary" & round(age) == 13) %>% arrange(school_type),
    aes(label = school_type), 
    family = "Times", seed = 810, nudge_x = .5, nudge_y = -.7) +
  geom_text_repel(
    data = fitted_ST %>% filter(school_type == "Intermediate secondary") %>% filter(age == max(age, na.rm = T)) %>% arrange(school_type),
    aes(label = school_type), 
    family = "Times", seed = 810, nudge_y = -1.3, nudge_x = 1.1) +
  geom_text_repel(
    data = fitted_ST %>% filter(school_type == "Lower secondary") %>% filter(age == max(age, na.rm = T)) %>% arrange(school_type),
    aes(label = school_type), 
    family = "Times", seed = 810, point.padding = 1, direction = "x") +
  theme(
    panel.grid.major.y = element_blank(),
    panel.grid.minor.y = element_blank(),
    panel.grid.major.x = element_line(linetype = "dashed", size = 0.3),
